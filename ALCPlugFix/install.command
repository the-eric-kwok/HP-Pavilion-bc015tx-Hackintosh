...
function cleanUpOldInstall() {
    ...
    sudo rm -rf /etc/ALCPlugFix/config.plist
}
...
then
    ...
    then
        ...
        sudo mkdir -p /etc/ALCPlugFix
        sudo cp "$inputFile" /etc/ALCPlugFix/config.plist || abort "Failed to copy config file"
    else
...
sudo launchctl load -w /Library/LaunchAgents/com.black-dragon74.ALCPlugFix.plist
